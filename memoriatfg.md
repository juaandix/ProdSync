...
| RNF-05 | Desplegabilidad | El sistema completo (backend + frontend + base de datos) arranca con `docker compose up --build` desde la raíz del monorepo. |
...
El sistema completo arranca con un solo comando desde la raíz del monorepo:
...
**Servicios Docker (docker-compose.yml en raíz):**

| Servicio | Imagen | Puerto | Descripción |
|----------|--------|--------|-------------|
| `db` | `postgres:16-alpine` | 5433 | Base de datos PostgreSQL |
| `backend` | Build local | 8080 | API Spring Boot |
| `seed` | `node:20-alpine` | — | Crea usuarios de prueba al arrancar |
| `frontend` | Build local | 3000 | Aplicación Next.js |

No es necesario tener Java ni Node.js instalados localmente para ejecutar la aplicación.

**Variables de entorno requeridas (`.env` en raíz):**

```yaml
ANTHROPIC_API_KEY: <clave de API de Anthropic, para el módulo IA>
```

**Variables de entorno internas (gestionadas por Docker Compose):**

```yaml
SPRING_DATASOURCE_URL: jdbc:postgresql://db:5432/prodsyncdb
...
- [x] **Docker Compose** en raíz: stack completo (backend + frontend + PostgreSQL + seed automático) con un solo comando
...
| `operator@test.com` | `password123` | ROLE_OPERATOR |
| `user@test.com` | `password123` | ROLE_USER |

> **Nota:** El usuario ADMIN lo crea el backend automáticamente al arrancar. Los usuarios OPERATOR y USER los crea el servicio `seed` de Docker Compose. También se puede ejecutar manualmente con `npm run seed` desde `prodsync-frontend`.
