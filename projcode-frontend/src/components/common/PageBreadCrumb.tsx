...
const PageBreadcrumb: React.FC = () => {
  ...
  return (
    <nav className="mb-6 w-full">
      <ol className="flex items-center gap-1.5 justify-end">
        <li>
          <Link className="inline-flex items-center gap-1.5 text-sm text-gray-500 dark:text-gray-400 hover:text-brand-500" href="/dashboard">
            Home
          </Link>
        </li>
        {pathSegments.map((segment, index) => {
          ...
          return (
            <li key={href} className="flex items-center">
              <svg className="w-4 h-4 stroke-current text-gray-400" viewBox="0 0 17 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M6.0765 12.667L10.2432 8.50033L6.0765 4.33366" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round" />
              </svg>
              {isLast ? (
                <span className="text-sm text-gray-800 dark:text-white/90 ml-1.5">{formattedSegment}</span>
              ) : (
                <Link className="inline-flex items-center gap-1.5 text-sm text-gray-500 dark:text-gray-400 hover:text-brand-500 ml-1.5" href={href}>
                  {formattedSegment}
                </Link>
              )}
            </li>
