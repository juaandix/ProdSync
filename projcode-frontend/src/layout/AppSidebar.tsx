...
const AppSidebar: React.FC = () => {
          ...
          {nav.subItems ? (
            ...
            >
              <span
                className={`${
                  openSubmenu?.type === menuType && openSubmenu?.index === index
                    ? "text-[#E93222] dark:text-[#E93222]"
                    : "text-white group-hover:text-[#E93222] dark:text-white dark:group-hover:text-[#E93222]"
                }`}
              >
                {nav.icon}
              </span>
                ...
                <>
                  <span
                    className={`menu-item-text ${
                      openSubmenu?.type === menuType && openSubmenu?.index === index
                        ? "text-[#E93222]"
                        : "text-white group-hover:text-[#E93222]"
                    }`}
                  >
                    {nav.name}
                  </span>
  ...
  return (
    <aside
      className={`fixed mt-16 flex flex-col lg:mt-0 top-0 px-5 left-0 bg-[#1E1E26] dark:bg-[#1E1E26] dark:border-gray-800 text-white h-screen transition-all duration-300 ease-in-out z-50 border-r border-[#1E1E26] text-lg
        ${isExpanded || isMobileOpen ? "w-[210px]" : isHovered ? "w-[210px]" : "w-[80px]"}
    ...
    >
      ...
      >
        <Link href="/dashboard">
          {isExpanded || isHovered || isMobileOpen ? (
            ...
            <Image src="/images/logo.png" alt="Softcode Logo" width={42} height={42} />
          )}
        </Link>
      </div>
      ...
      {(isExpanded || isHovered || isMobileOpen) && (
        <div className="-mx-5 w-[calc(100%+2.5rem)]">
          <Image
            src="https://www.softcode.es/wp-content/uploads/2024/09/detalle-banner-1.png"
            alt="Softcode detail"
            width={210}
            height={207}
