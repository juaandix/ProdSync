...
const CreateTaskForm: React.FC<CreateTaskFormProps> = ({ projectId, onTaskCreated, onClose }) => {
  ...
  const estimacion   = useWatch({ control, name: 'estimacion' });
  const storyPoints  = useWatch({ control, name: 'storyPoints' });
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      ...
      <RoleGuard roles={['ADMIN', 'OPERATOR']}>
        <AiAssignmentPanel
          ...
          estimacion={typeof estimacion === 'number' ? estimacion : undefined}
