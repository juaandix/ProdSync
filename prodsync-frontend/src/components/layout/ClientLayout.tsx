...
const ClientLayout: React.FC<ClientLayoutProps> = ({ children }) => {
  ...
  const mainContentMargin = isExpanded || isHovered ? "lg:ml-[185px]" : "lg:ml-[68px]";
  ...
  return (
    <div className="flex h-screen overflow-hidden">
      ...
      <div className={`relative flex flex-1 flex-col overflow-hidden ${mainContentMargin}`}>
        <div className="flex-1 overflow-y-auto overflow-x-hidden bg-[#1E1E26] text-white">
          <main className="px-4 sm:px-6 pt-4 sm:pt-6">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                <button
                  onClick={handleToggle}
                  className="flex items-center justify-center w-9 h-9 text-white border border-white/20 rounded-lg hover:bg-white/10 shrink-0"
